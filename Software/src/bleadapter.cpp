...
bool ble_available() { return ble_ready || setup_required; }

void ble_setWorkAvailable() { ble_ready = true; }
...
void ble_setup() {
  ...
  */
  ...
  aci_state.aci_pins.spi_clock_divider =
      SPI_CLOCK_DIV8;  // SPI_CLOCK_DIV8  = 2MHz SPI speed
  ...
  // We reset the nRF8001 here by toggling the RESET line connected to the
  // nRF8001 and initialize the data structures required to setup the nRF8001
...
void ble_loop() {
  ...
  if (lib_aci_event_get(&aci_state, &aci_data)) {
    ...
    switch (aci_evt->evt_opcode) {
      /**
        As soon as you reset the nRF8001 you will get an ACI Device Started
        Event
      */
      case ACI_EVT_DEVICE_STARTED: {
        ...
        switch (aci_evt->params.device_started.device_mode) {
          ...
          case ACI_DEVICE_STANDBY:
            ...
            // lib_aci_broadcast(0/* in seconds */, 0x0100 /* advertising
            // interval 100ms */);
            lib_aci_connect(0, 0x0100);
            // While broadcasting (non_connectable) interval of 100ms is the
            // minimum possible
      ...
      case ACI_EVT_CMD_RSP:
        ...
        if (ACI_STATUS_SUCCESS != aci_evt->params.cmd_rsp.cmd_status) {
          ...
          // commands will have status code of ACI_STATUS_SCUCCESS for a
          // successful command
          ...
          if (ACI_CMD_GET_TEMPERATURE == aci_evt->params.cmd_rsp.cmd_opcode) {
            ...
            Serial.println(aci_evt->params.cmd_rsp.params.get_temperature
                               .temperature_value,
                           HEX);
            // Swap temperature bytes
            uint16_t temperature = aci_evt->params.cmd_rsp.params
                                       .get_temperature.temperature_value;
      ...
      case ACI_EVT_HW_ERROR:
        ...
        for (uint8_t counter = 0; counter <= (aci_evt->len - 3); counter++) {
          Serial.write(aci_evt->params.hw_error
                           .file_name[counter]);  // uint8_t file_name[20];
        }
        Serial.println();
        ble_pipeEvent_Cbk();
        break;
    ...
    if (!setup_required) {
      ...
      /* setup_required is set to true when the device starts up and enters
         setup mode. It indicates that do_aci_setup() should be called. The flag
         should be cleared if do_aci_setup() returns
